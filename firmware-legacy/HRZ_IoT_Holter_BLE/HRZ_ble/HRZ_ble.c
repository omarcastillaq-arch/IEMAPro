...
hrz_ecgs_t m_ecgs;              /**< Structure used to identify the ECG service. */
volatile bool buffer_is_free = true;     /** Flag to indicate when the BLE buffer is free*/
...
void services_init(void)
{
  ...
  err_code = hrz_ecgs_init(&m_ecgs, &ecgs_init);
  APP_ERROR_CHECK(err_code);
}
...
void ble_evt_dispatch(ble_evt_t * p_ble_evt)
{
    ...
    hrz_ecgs_on_ble_evt(&m_ecgs, p_ble_evt);
}
...
void ble_stack_init(void)
{
  ret_code_t err_code;
    ...
    // Configure maximum packets per connection event
    memset(&ble_cfg, 0, sizeof ble_cfg);
